...
class ProfileTextInputLayout @JvmOverloads constructor(
    context: Context,
    ...
    private fun init(attrs: AttributeSet?) {
        View.inflate(context, R.layout.profile_text_input_layout, this)

        val ta = context.obtainStyledAttributes(attrs, R.styleable.ProfileTextInputLayout)
        ...
        try {
            ...
            val textView = findViewById<EditText>(R.id.text).apply {
                setText(ta.getString(R.styleable.ProfileTextInputLayout_android_text))
                hintView.isVisible = text.isNotEmpty()
                hint = ta.getString(R.styleable.ProfileTextInputLayout_android_hint)
                imeOptions =
                    ta.getInt(R.styleable.ProfileTextInputLayout_android_imeOptions, imeOptions)
                inputType =
                    ta.getInt(R.styleable.ProfileTextInputLayout_android_inputType, inputType)
                filters = arrayOf(
                    *this.filters,
                    InputFilter.LengthFilter(
                        ta.getInt(
                            R.styleable.ProfileTextInputLayout_android_maxLength,
                            256
                        )
                    )
                )
            ...
            findViewById<ImageView>(R.id.icon).apply {
                setImageDrawable(ta.getDrawable(R.styleable.ProfileTextInputLayout_android_icon))

                val tintColor =
                    ta.getColor(R.styleable.ProfileTextInputLayout_tint, Color.TRANSPARENT)
            ...
            clearView.apply {
                isVisible = clearEnabled && textView.text.isNotEmpty()
                setOnClickListener {
                    textView.setText("")
                }
            }
    ...
    val textField: EditText = findViewById(R.id.text)
